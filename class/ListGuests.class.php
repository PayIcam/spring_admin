		...
		if($this->countGuests){
	        foreach ($this->guestsList as $guest) { ?>
				<tr class="<?php echo ($guest['is_icam']==1)?'':'warning' ?>">
				  ...
				  <td><?php echo (empty($motclef) || empty($guest['email']))? $guest['email']."&nbsp;": preg_replace('/('.$motclef.')/i', "<strong>$1</strong>", $guest['email']); ?></td>
				  <td><?php echo (empty($motclef) || empty($guest['promo']))? $guest['promo']."&nbsp;": preg_replace('/('.$motclef.')/i', "<strong>$1</strong>", $guest['promo']); ?></td>
				  <td><?php 
				  ...
				  <td>
				    <div class="pull-right">
				      ...
				      <a href="admin_liste_guests.php?del_guest=<?php echo $guest['id']; ?>" title="Supprimer l'utilisateur #<?php echo $guest['id']; ?>" onclick="return confirm('Voulez-vous vraiment supprimer cet invité et ses invités ?');"><i class="icon-trash"></i></a>              
				    </div>
				  </td>
				</tr>
	        	<?php
