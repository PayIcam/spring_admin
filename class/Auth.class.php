<?php class Auth{
    ...
    function loginUsingCas($ticket){
        ...
        $userEmail = $CAS->authenticate($ticket,Config::get('galadesicam_url'));
        ...
        if (!empty($userEmail) && $DB->findCount(self::TABLE_NAME,array('email'=>$userEmail),'email') == 1) {
            ...
            Functions::setFlash("Vous ne faites pas parti des administrateur de l'Administration du projet Gala des Icam.<br>Faites la demande aux responsables au besoin.",'warning');
            header('Location:connection.php');exit;
        }
        return false;
    }
    ...
    public static function validateToken($token, $nom = '', $temps = 600, $referer = ''){
        if (empty($referer)){
            $referer = Config::get('galadesicam_url').basename($_SERVER['REQUEST_URI']);
        }
            ...
            if($_SESSION['tokens'][$nom.'_token'] == $token)
                if($_SESSION['tokens'][$nom.'_token_time'] >= (time() - $temps)){
                    ...
                    elseif(empty($_SERVER['HTTP_REFERER']))
                        ...
                        Functions::setFlash('<strong>erreur HTTP_REFERER</strong> '.$referer.' != '.$_SERVER['HTTP_REFERER'].' vous ne vous êtes pas planté dans votre config ?! config.php','danger');
                }
        return false;
    }
